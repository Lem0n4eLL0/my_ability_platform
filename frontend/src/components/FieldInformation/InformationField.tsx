...
export const InformationField = (props: IInformationField) => {
  const { children, canChange, canAdd, lable } = props;
  return (
    <div className={style['information-field']}>
      {(lable || canAdd) && (
        <div
          className={clsx(
            style['information-field__top'],
            !lable && style['information-field__top_end']
          )}
        >
          {lable && <h3 className={style['information-field__title']}>{lable}</h3>}
          {canAdd && (
            <button onClick={canAdd} className={style['information-field__add-button']}>
              добавить
            </button>
          )}
        </div>
      )}
      <div className={style['information-field__content']}>
        {children}
        {canChange && (
          <button onClick={canChange} className={style['information-field__edit']}>
