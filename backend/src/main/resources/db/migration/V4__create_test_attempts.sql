...
-- Ответы на вопросы в рамках попытки
CREATE TABLE IF NOT EXISTS attempt_questions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Детализация количества вопросов по типам в критериях теста
CREATE TABLE IF NOT EXISTS questions_number_by_type_criteria (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    test_criteria_id UUID NOT NULL,
    question_type VARCHAR NOT NULL,
    required_count INTEGER NOT NULL CHECK (required_count >= 0),
    CONSTRAINT fk_cqtc_criteria
        FOREIGN KEY (test_criteria_id)
        REFERENCES test_criteria(id)
        ON DELETE CASCADE,
